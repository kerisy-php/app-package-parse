...
BinaryXmlParser = (function () {
    ...
    BinaryXmlParser.prototype.readU16 = function () {
        for (var byte = [], index = this.cursor; index < this.cursor + 2; index++) {
            byte.push(this.buffer[index]);
        }
        this.cursor += 2;
        return byte[0] + byte[1] * Math.pow(2, 8)
    };

    BinaryXmlParser.prototype.readU32 = function () {
        for (var byte = [], result = 0, index = this.cursor; index < this.cursor + 4; index++) {
            byte.push(this.buffer[index]);
            result += byte[index - this.cursor] * Math.pow(2, 8 * (index - this.cursor))
        }
        this.cursor += 4;
        return "4294967295" == result && (result = -1), result
    };

    BinaryXmlParser.prototype.readS32 = function () {
        var bufferLength = this.buffer.length;
        if (this.cursor < bufferLength) {
            var value;
            this.cursor + 2 < bufferLength && (value = this.buffer[this.cursor + 2] << 16);
            this.cursor + 1 < bufferLength && (value |= this.buffer[this.cursor + 1] << 8);
            value |= this.buffer[this.cursor];
            this.cursor + 3 < bufferLength && (value += this.buffer[this.cursor + 3] << 24 >>> 0);
            this.cursor += 4;
            var f = 2147483648 & value;
            return f ? (4294967295 - value + 1) * -1 : value;
        }
    };
    ...
    BinaryXmlParser.prototype.readTypedValue = function () {
        ...
        switch (dataType) {
            ...
            case TypedValue.TYPE_REFERENCE:
                id = this.readU32();
                typedValue.value = "@" + (id.toString(16)).toUpperCase();
    ...
    BinaryXmlParser.prototype.readString = function (encoding) {
        function getStringFromBytes(bytes, start, end) {
            var string = "";
            bytes = bytes.slice(start, end);
            for (index = 0; index < bytes.length; index += 2) {
                string += String.fromCharCode(bytes[index] + 256 * bytes[index + 1]);
            }
            return string;
        }
        ...
        switch (encoding) {
            ...
            case "ucs2":
                ...
                byteLength = 2 * stringLength;
                bytes = this.readBytes(byteLength);
                string = getStringFromBytes(bytes, 0, bytes.length);
                this.readU16();
                return string;
            default:
    ...
    BinaryXmlParser.prototype.readBytes = function (len) {
        var bytes = this.buffer.slice(this.cursor, this.cursor + len);
        this.cursor += len;
        return bytes;
    };
    ...
    BinaryXmlParser.prototype.readStringPool = function (header) {
        ...
        var stringStart = this.cursor + header.stringsStart - header.headerSize;
        offsets = [];
        for (_i = 0, _ref = header.stringCount; 0 <= _ref ? _i < _ref : _i > _ref; 0 <= _ref ? _i++ : _i--) {
            offsets.push(stringStart + this.readU32());
        }
        encoding = header.flags & StringFlags.UTF8 ? 'utf-8' : 'ucs2';
        this.cursor = anchor + header.stringsStart - header.headerSize;

        var offset = -1;
        for (_j = 0; _j < offsets.length; _j++) {
            if (offsets[_j] != offset) {
                this.cursor = offset = offsets[_j];
                var string = this.readString(encoding);
