ApkResourceFinder = (function () {
    ...
    var ToString = function (bytes, style) {
        if (style == "X4") {
            return ToX4String(bytes);
        }
        return bytes.toString();
    };

    var ContainsKey = function (bytes, key) {
        return bytes[key] !== undefined;
    };

    var Add = function (bytes, key, value) {
        bytes[key] = value;
    };
    ...
    BufferReader.prototype.ReadInt16 = function () {
        for (var byte = [], index = this.BaseStream.Position; index < this.BaseStream.Position + 2; index++) {
            byte.push(this._buffer[index]);
        }
    ...
    BufferReader.prototype.ReadInt32 = function () {
        for (var byte = [], result = 0, index = this.BaseStream.Position; index < this.BaseStream.Position + 4; index++) {
            byte.push(this._buffer[index]);
            result += byte[index - this.BaseStream.Position] * Math.pow(2, 8 * (index - this.BaseStream.Position))
        }
    ...
    BufferReader.prototype.ReadByte = function () {
        var temp = this._buffer[this.BaseStream.Position];
    ...
    var ApkResourceFinder = {
        ...
        processResourceTable: function (buffer) {
            ...
            while (true)
            {
                ...
                {
                    // Process the package
                    console.log("Processing package " + realPackageCount + " ...");
            ...
            for (var i = 0; i < entryCount; ++i)
            {
                ...
                if ((entry_flag & FLAG_COMPLEX) == 0)
                {
                    ...
                    if (value_dataType == TYPE_STRING)
                    {
                        ...
                        //console.log(", data: " + valueStringPool[value_data] + "");
        ...
        processStringPool: function (data) {
            ...
            for (var i = 0; i < stringCount; i++)
            {
                ...
                //console.log("Parsed value: " + strings[i]);
        ...
        processTypeSpec: function (data) {
            ...
            //console.log("Processing type spec " + typeStringPool[id - 1]);
